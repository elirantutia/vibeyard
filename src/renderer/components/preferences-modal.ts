...
type Section = 'general' | 'appearance' | 'browser' | 'shortcuts' | 'profiles' | 'setup' | 'about';
...
export function showPreferencesModal(): void {
  ...
  function renderProfilesSection(container: HTMLElement) {
  ...
  cleanupFn = () => {
    ...
    zoomPrefUnsub = null;
    if (defaultProviderSelect) defaultProviderSelect.destroy();
    if (profileDefaultSelect) profileDefaultSelect.destroy();
