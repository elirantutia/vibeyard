...
function render(): void {
  ...
  if (others.length) {
    if (active) {
      ...
      label.textContent = t('sidebar.allProjects');
...
function confirmRemoveProject(project: ProjectRecord): void {
  ...
  const parts: string[] = [];
  if (historyCount > 0) {
    const entryNoun = historyCount === 1 ? 'entry' : 'entries';
    parts.push(t('sidebar.removeConfirm.sessionsHistory', { count: historyCount, noun: entryNoun }));
  }
  if (taskCount > 0) {
    const taskNoun = taskCount === 1 ? 'task' : 'tasks';
    parts.push(t('sidebar.removeConfirm.kanbanTasks', { count: taskCount, noun: taskNoun }));
  }
