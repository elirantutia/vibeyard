...
- `components/title-bar.ts` + `styles/title-bar.css` — **Windows-only** custom in-app title bar. `main.ts` sets `titleBarStyle: 'hidden'` + `titleBarOverlay` (dark `#101116` tint, 36px) on `isWin`, so Windows shows no native title/menu bar; `createAppMenu` returns early on `isWin`, and the renderer builds a dark 36px nav bar (`#title-bar` above the `#app-body` row, `-webkit-app-region: drag`) with the brand + File/Edit/View dropdowns, reusing the shared `.tab-context-menu` classes + `tab-bar/menu.ts` single-menu tracker (one open menu, click-outside/Escape dismissal). Menus are built on click so `preferences.debugMode` is read live. File uses existing `promptNewProject`/`quickNewSession`/`closeSessionWithConfirm` + new `menu.quitApp`; Edit dispatches `window.vibeyard.menu.runEditAction(action)` (new `menu:edit-action` IPC → `focusedContents()[action]()`, same behavior as the old native Windows Edit menu) after restoring focus to the previously focused element (queueMicrotask) so webContents edit commands target the terminal/input, not the menu button; View uses `appState.toggleSwarm`/`toggleInspector`, plus debug items (`toggleDebugPanel`, `menu:toggleDevTools`, `menu:reloadMainWindow`) when debug mode is on. macOS/Linux keep the native title bar + system menu bar. All shortcuts still work on Windows because native menu accelerators are disabled (`registerAccelerator: false`).
